        ...
        for subdir in subdirs:
            ...
            try:
                ...
                for sci_path in hdf5_path:
                    ...
                    tmp_dict['unixtime'] = np.ndarray.flatten(hdf.get('depth').get('time')[:])
                    ...
                    logger.info('Converting timestamps to unixtime')
                    df['timestamp'] = df['datenum'].apply(lambda matlab_datenum: datetime.fromordinal(int(matlab_datenum)) + timedelta(days=matlab_datenum%1) - timedelta(days = 366))
                    df['unixtime'] = df['datenum'].apply(lambda matlab_datenum: (datetime.fromordinal(int(matlab_datenum)) + timedelta(days=matlab_datenum%1) - timedelta(days = 366)).timestamp())
